...
export class TestMainMenuEventManager extends EventManager{
  ...
  constructor(connectionService: ConnectionService, gameService: GameService, private playerService:PlayerService, private router: Router){
    super(connectionService, gameService);
  }

  public init() {
    this.subscribe(this.START, new TestStartGameEvent(this.gameService, this.playerService));
    this.subscribe(this.READY_START, new TestReadyGameEvent(this.router));
  }
